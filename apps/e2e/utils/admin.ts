...
export async function verifyCaptureOnAdmin(fingerprint: string): Promise<void> {
  ...
  if (!adminUser || !adminPassword) {
    throw new Error("ADMIN_USER / ADMIN_PASSWORD missing — set them in .env");
  }
  const adminUrl = (process.env.ADMIN_URL || "https://dev-admin.treetracker.org").replace(/\/$/, "");
  ...
  try {
    ...
    // ── /verify ────────────────────────────────────────────────────────────
    await driver.url(`${adminUrl}/verify`);
